          ...
          for (var m2 in to) {
            ...
            if (d1 < d2) {
              if (!used.includes(to[m2]['to_user_id'])) {
                ...
                html += '<div class="messages" user_id=' + to[m2]['to_user_id'] + '><div class="avatar"><img src="../assets/images/user_picture/' + to[m2]['to_user_id'] + '.jpeg" alt=""> </div><div class="friend"><div class="user">' + to[m2]['name'] + '</div><div class="timedisplay">Last sent on: ' + to[m2]['date_updated'].replace('T', ' ').replace(".000Z", " ") + '</div> </div> </div>'
              }
            } else {
              if (!used.includes(from[m1]['from_user_id'])) {
                ...
                html += '<div class="messages" user_id=' + from[m1]['from_user_id'] + '><div class="avatar"><img src="../assets/images/user_picture/' + from[m1]['from_user_id'] + '.jpeg" alt=""> </div><div class="friend"><div class="user">' + from[m1]['name'] + '</div><div class="timedisplay">Last received on: ' + from[m1]['date_updated'].replace('T', ' ').replace(".000Z", " ") + '</div> </div> </div>'
              }
            }
          }
        }
    ...
    success: function (response) {
      if (response.status == "success") {
        ...
        for (var msg in msgdata) {
          if (msgdata[msg]['to_user_id'] == response.userid) {
        ...
        $("#userimgmsg").attr('src', '../assets/images/user_picture/' + from_user_id + '.jpeg');
      ...
      if (response.status == "success") {
        ...
        if (data['sent'] && data['sent'].length > 0) {
          ...
          for (var index in data['sent']) {
            html += '<div class="messages" status="' + data['sent'][index]['request_status'] + '" tabid="' + data['sent'][index]['workout_id'] + '" user_id=' + data['sent'][index]['to_user_id'] + '> <div class="avatar"> <img src="../assets/images/user_picture/' + data['sent'][index]['to_user_id'] + '.jpeg" alt=""> </div><div class="friend"> <div class="user">' + data['sent'][index]['name'] + '</div> <div class="timedisplay">' + data['sent'][index]['date_sent'].replace('T', ' ').replace(".000Z", " ") + '</div>  </div> </div>';
          }
          $("#matchsent").html(html);
        }
      } else {
...
function showSearchResults(no) {
  if (filterResult[no]) {
    ...
    var html = '<img class="user" src="../assets/images/user_picture/' + info.reg_id + '.jpeg" /><div class="profile"><div class="name">' + info.name + '</div></div>';
...
$(document).ready(function () {
  ...
  $("#matchreceived").on("click", ".messages", function () {
    ...
    var html = '<img class="user" src="../assets/images/user_picture/' + userid + '.jpeg" /> <div class="profile"><div class="name">' + username + '</div> <div class="local"> </div></div>';
